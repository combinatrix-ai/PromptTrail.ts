...
> extends Template<TInput, TOutput> {
  ...
  async execute(session: Session<TInput>): Promise<Session<TOutput>> {
    ...
    // First try to use the ai-sdk implementation
    // If it fails, we'll fall back to the original implementation
    try {
      // Get the ai-sdk model from our model implementation
      const aiSdkModel = this.model.getAiSdkModel();
      
      // Convert session messages to ai-sdk format
      const aiMessages = this.model.convertSessionToAiSdkMessages(session as Session);
      
      // Schema format is determined directly in the generateObject call below
      
      try {
        // Use ai-sdk's generateObject for structured output
        // We need to cast the entire function call due to type incompatibilities
        // Using type assertion with a more specific function type
        const result = await (generateObject as unknown as (options: Record<string, unknown>) => Promise<{object: unknown}>)({
          model: aiSdkModel,
          messages: aiMessages,
          schema: this.isZodSchema 
            ? (this.schema as z.ZodType) 
            : {
                type: 'object',
                properties: this.nativeSchema.properties,
                required: this.nativeSchema.required || [],
              },
          schemaName: this.functionName,
        });
        
        const object = result.object;
        
        // Create a new session with the structured output
        const resultSession = await session.addMessage({
          type: 'assistant',
          content: JSON.stringify(object, null, 2),
          metadata: createMetadata().set('structured_output', object),
        });
        
        // Return the session with structured output in metadata
        return resultSession.updateMetadata({
          structured_output: object,
        }) as unknown as Session<TOutput>;
      } catch (innerError) {
        console.warn('Error using ai-sdk generateObject, falling back to legacy implementation:', innerError);
        // Continue to fallback implementation
      }
    } catch (aiSdkError) {
      console.warn('Error in ai-sdk implementation, using legacy implementation:', aiSdkError);
      // Continue to fallback implementation
    }
  ...
  `;
      ...
      // @ts-expect-error Using any type for backward compatibility
      const resultSession = await guardrailTemplate.execute(systemSession);
