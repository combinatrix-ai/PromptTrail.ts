  ...
  for (const [key, prop] of Object.entries(schema.properties)) {
    ...
    if (propObj.type === 'string') {
      ...
      const nestedSchema: SchemaType = {
        properties: propObj.properties as unknown as Record<
          string,
          PropertySchema
        >,
        required: propObj.required || [],
      };
      shape[key] = schemaTypeToZod(nestedSchema).describe(
        propObj.description || '',
      );
    } else if (propObj.type === 'array') {
      // Handle arrays with type assertion for items
      const itemsObj = (propObj as unknown as { items?: { type: string } })
        .items;
...
> extends Template<TInput, TOutput> {
  ...
  constructor(options: {
    ...
    this.schemaDescription =
      ...
      'Generate structured data according to the schema';
  ...
  async execute(session: Session<TInput>): Promise<Session<TOutput>> {
    ...
    const modelConfig = (
      this.model as unknown as { config: Record<string, unknown> }
    ).config;
    if (!modelConfig) {
      throw new Error(
        'Model configuration is required for AISDKSchemaTemplate',
      );
    }
  ...
  private async executeNonStreaming(
    ...
    try {
      ...
      const { object } = await (
        generateObject as unknown as (options: {
          model: unknown;
          schema: z.ZodType;
          schemaDescription?: string;
          messages: unknown;
        ...
        messages,
        temperature:
          (this.model as unknown as { config: { temperature?: number } }).config
            .temperature || 0.1,
      });
      ...
      newMetadata.set(
        'structured_output' as keyof TOutput,
        object as unknown as TOutput[keyof TOutput],
      );
      ...
      throw new Error(
        `Failed to generate structured output: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async executeStreaming(
    ...
    try {
      ...
      // Using unknown type to work around AI SDK type issues
      const { object } = await (
        streamObject as unknown as (options: {
          model: unknown;
          schema: z.ZodType;
          schemaDescription?: string;
          messages: unknown;
      ...
      throw new Error(
        `Failed to stream structured output: ${(error as Error).message}`,
      );
    }
  }
}
