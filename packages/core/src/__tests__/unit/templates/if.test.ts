...
describe('If Template', () => {
  ...
  it('should handle conditions based on message content', async () => {
    ...
    const elseTemplate = new AssistantTemplate('I did not understand your message.');
    ...
    expect(questionMessages[1].content).toBe('I did not understand your message.');
  });
  ...
  it('should handle nested if templates', async () => {
    ...
    const nonAdminResultSession = await outerIfTemplate.execute(nonAdminSession);
  ...
  it('should handle dynamically determined template paths', async () => {
    ...
    const messageTypeHandler = new IfTemplate({
      condition: (session) => session.metadata.get('messageType') === 'greeting',
      thenTemplate: greetingTemplate,
      elseTemplate: new IfTemplate({
        condition: (session) => session.metadata.get('messageType') === 'question',
    ...
    const questionResultSession = await messageTypeHandler.execute(questionSession);
    const questionMessages = Array.from(questionResultSession.messages);
    ...
    const defaultResultSession = await messageTypeHandler.execute(defaultSession);
