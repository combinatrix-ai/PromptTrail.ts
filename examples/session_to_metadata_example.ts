...
export function createTransformer<
  ...
  TOutput extends Record<string, unknown> = TInput
>(
  transformFn: (session: Session<TInput>) => Promise<Session<TOutput>> | Session<TOutput>
...
function quickSort(arr: number[]): number[] {
  ...
  const analysisTemplate = new LinearTemplate()
    .addSystem("You're a code reviewer. Organize your analysis with ## headings for Summary, Strengths, Weaknesses, and Suggestions.")
    .addUser(`Analyze this TypeScript code:\n\`\`\`typescript\n${codeToAnalyze}\n\`\`\``)
    .addAssistant({ model: model })
    // This is how the API would look once implemented:
    .addTransformer(extractMarkdown({
      headingMap: {
 ...
 */
async function main() {
  console.log("NOTE: This is a conceptual example of the Session to Metadata conversion feature.");
  console.log("The actual implementation would require adding the feature to the core library.");
  console.log("This file demonstrates the proposed API design and usage patterns.");
