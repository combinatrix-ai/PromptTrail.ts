...
async function main() {
  ...
  const productTemplate = new LinearTemplate()
    ...
    `)
    .addUser("Write a product description for a new smartphone called 'Quantum X' with advanced AI features, a 6.7-inch display, and 48-hour battery life.", "");
  ...
  if (guardrailInfo) {
    ...
    if (guardrailInfo.validationResults) {
      ...
      guardrailInfo.validationResults.forEach((result, index: number) => {
        console.log(`- Validator ${index + 1}: ${result.passed ? 'PASSED' : 'FAILED'} ${result.score ? `(Score: ${result.score})` : ''}`);
      });
    }
  } else {
